    ...
    public override void mapNetworkDataToStructure(int packetNumber, string line)
        {
            ...
            if (isNewPacket(packetNumber))
            {
                ...
                if (line != null && !line.Equals(string.Empty))
                {
                    ...
                    {
                        ACCELERATION_X = (float) Convert.ToDecimal(splitMessage[0], cultureInfo);
                        ACCELERATION_Y = (float) Convert.ToDecimal(splitMessage[1], cultureInfo);
                        ACCELERATION_Z = (float) Convert.ToDecimal(splitMessage[2], cultureInfo);
                    }
                }
            }
      ...
      // message += Convert.ToDecimal(GYRO_W, cultureInfo).ToString(cultureInfo) + EasyWiFiConstants.SPLITMESSAGE_COMMA;
            message += Convert.ToDecimal(ACCELERATION_X, cultureInfo).ToString(cultureInfo) + EasyWiFiConstants.SPLITMESSAGE_COMMA;
            message += Convert.ToDecimal(ACCELERATION_Y, cultureInfo).ToString(cultureInfo) + EasyWiFiConstants.SPLITMESSAGE_COMMA;
            message += Convert.ToDecimal(ACCELERATION_Z, cultureInfo).ToString(cultureInfo) + EasyWiFiConstants.SPLITMESSAGE_NEWLINE;
