...
namespace EasyWiFi.Core
{
    public class GyroControllerType : BaseControllerType
  {
    CultureInfo cultureInfo = new CultureInfo("en-US");
        ...
        public override void mapNetworkDataToStructure(int packetNumber, string line)
        {
            ...
            if (isNewPacket(packetNumber))
            {
                ...
                if (line != null && !line.Equals(string.Empty))
                {
                    ...
                    {
            GYRO_W = (float) Convert.ToDecimal(splitMessage[0], cultureInfo);
            GYRO_X = (float) Convert.ToDecimal(splitMessage[1] , cultureInfo);
            GYRO_Y = (float) Convert.ToDecimal(splitMessage[2], cultureInfo);
            GYRO_Z = (float) Convert.ToDecimal(splitMessage[3], cultureInfo);
                    }
                }
            }
      ...
      message += Convert.ToDecimal(GYRO_W, cultureInfo).ToString(cultureInfo) + EasyWiFiConstants.SPLITMESSAGE_COMMA;
      message += Convert.ToDecimal(GYRO_X, cultureInfo).ToString(cultureInfo) + EasyWiFiConstants.SPLITMESSAGE_COMMA;
      message += Convert.ToDecimal(GYRO_Y, cultureInfo).ToString(cultureInfo) + EasyWiFiConstants.SPLITMESSAGE_COMMA;
      message += Convert.ToDecimal(GYRO_Z, cultureInfo).ToString(cultureInfo) + EasyWiFiConstants.SPLITMESSAGE_NEWLINE;
