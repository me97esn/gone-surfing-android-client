        ...
        public override void mapNetworkDataToStructure(int packetNumber, string line)
        {
            ...
            if (isNewPacket(packetNumber))
            {
                ...
                if (line != null && !line.Equals(string.Empty))
                {
                    ...
                    {
                        JOYSTICK_HORIZONTAL = (float) Convert.ToDecimal(splitMessage[0], new CultureInfo("en-US"));
                        JOYSTICK_VERTICAL = (float) Convert.ToDecimal(splitMessage[1], new CultureInfo("en-US"));
                    }
                }
            }
