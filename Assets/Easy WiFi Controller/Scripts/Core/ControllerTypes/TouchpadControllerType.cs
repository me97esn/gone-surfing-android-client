...
namespace EasyWiFi.Core
{
    ...
    public float POSITION_HORIZONTAL;
        ...
        public override void mapNetworkDataToStructure(int packetNumber, string line)
        {
            ...
            if (isNewPacket(packetNumber))
            {
                ...
                if (line != null && !line.Equals(string.Empty))
                {
                    ...
                    {
                        POSITION_HORIZONTAL = (float) Convert.ToDecimal(splitMessage[0], cultureInfo);
                        POSITION_VERTICAL = (float) Convert.ToDecimal(splitMessage[1], cultureInfo);
      ...
      message += Convert.ToDecimal(POSITION_HORIZONTAL, cultureInfo).ToString(cultureInfo) + EasyWiFiConstants.SPLITMESSAGE_COMMA;
            message += Convert.ToDecimal(POSITION_VERTICAL, cultureInfo).ToString(cultureInfo) + EasyWiFiConstants.SPLITMESSAGE_COMMA;
